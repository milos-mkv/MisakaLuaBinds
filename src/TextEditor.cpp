...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Z)))
			...
			Paste();
		else if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_F)))
			{
				mFindOpened = true;
			}
		...
		while (lineNo <= lineMax)
		{
			...
			if (mState.mCursorPosition.mLine == lineNo)
			{
				...
				if (mHighlightLine && !HasSelection()) {
					...
					drawList->AddRectFilled({start.x + 85, start.y}, end, mPalette[(int)(focused ? PaletteIndex::CurrentLineFill : PaletteIndex::CurrentLineFillInactive)]);
					drawList->AddRect({start.x + 85, start.y}, end, mPalette[(int)PaletteIndex::CurrentLineEdge], 1.0f);
				}
			...
			// side bar
			if (mSidebar) {
				drawList->AddRectFilled(ImVec2(lineStartScreenPos.x + scrollX, lineStartScreenPos.y), ImVec2(lineStartScreenPos.x + scrollX + mTextStart - 5.0f, lineStartScreenPos.y + mCharAdvance.y), ImGui::GetColorU32({ 0.061, 0.061, 0.061, 1.0f}));
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	if (mFindOpened)
	{
		...
		for (int i = 0; i < m_shortcuts.size(); i++) {
			...
			std::cout << sct.Key1 << ":" << sct.Key2 << std::endl;
			// SDL_Scancode sc1 = SDL_GetScancodeFromKey();
